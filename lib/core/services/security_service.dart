...
class SecurityService {
  ...
  static const _recoveryFailedAttemptsKey = 'arus_pin_recovery_failed_attempts_v1';
  ...
  Future<bool> hasRecoveryCode() async => (await _storage.read(key: _recoveryVerifierKey))?.isNotEmpty == true;
  ...
  Future<bool> verifyPin(String pin) async {
    ...
    if (attempts >= 5) {
      ...
      final until = DateTime.now().toUtc().add(Duration(seconds: 30 * (1 << exponent)));
      await _storage.write(key: _lockUntilKey, value: until.toIso8601String());
    }
    return false;
  }

  Future<DateTime?> pinLockedUntil() => _activeLockUntil(key: _lockUntilKey);
  ...
  Future<bool> verifyRecoveryCode(String code) async {
    ...
    try {
      ...
      if (payload['v'] == 1 && payload['salt'] is String && payload['hash'] is String && _isValidRecoveryCode(normalized)) {
  ...
  Future<DateTime?> recoveryLockedUntil() => _activeLockUntil(key: _recoveryLockUntilKey);

  Future<String?> resetPinWithRecoveryCode({required String recoveryCode, required String newPin}) async {
    _validatePin(newPin);
  ...
  Future<void> setBiometricEnabled(bool enabled) async {
    if (enabled && !await canUseBiometrics()) throw StateError('Biometrik tidak tersedia pada perangkat ini.');
  ...
  Future<bool> authenticateBiometric() async {
    ...
    try {
      ...
      _lastBiometricError = switch (error.code) {
        LocalAuthExceptionCode.userCanceled || LocalAuthExceptionCode.systemCanceled || LocalAuthExceptionCode.userRequestedFallback => null,
        LocalAuthExceptionCode.noBiometricsEnrolled => 'Belum ada sidik jari/biometrik yang terdaftar untuk aplikasi.',
        LocalAuthExceptionCode.noBiometricHardware => 'Sensor biometrik tidak terdeteksi oleh Android.',
        LocalAuthExceptionCode.biometricHardwareTemporarilyUnavailable => 'Sensor biometrik sedang tidak tersedia. Coba lagi.',
        LocalAuthExceptionCode.temporaryLockout => 'Biometrik dikunci sementara karena terlalu banyak percobaan. Coba lagi nanti.',
        LocalAuthExceptionCode.biometricLockout => 'Biometrik dikunci oleh perangkat. Buka kunci HP sekali lalu coba lagi.',
        LocalAuthExceptionCode.noCredentialsSet => 'Kunci layar perangkat belum siap untuk autentikasi biometrik.',
        LocalAuthExceptionCode.uiUnavailable => 'Prompt biometrik tidak dapat ditampilkan oleh perangkat.',
        LocalAuthExceptionCode.authInProgress => 'Permintaan biometrik lain masih berjalan.',
        LocalAuthExceptionCode.timeout => 'Pembacaan biometrik berakhir karena batas waktu.',
        LocalAuthExceptionCode.deviceError || LocalAuthExceptionCode.unknownError => error.description ?? 'Perangkat gagal memproses autentikasi biometrik.',
      };
      return false;
    } catch (_) {
  ...
  Future<void> _writeRecoveryVerifier(String normalizedCode) async {
    ...
    await _storage.write(key: _recoveryVerifierKey, value: jsonEncode({'v': 1, 'salt': base64UrlEncode(salt), 'hash': base64UrlEncode(hash)}));
  }

  Future<void> _recordRecoveryFailure() async {
    final attempts = (int.tryParse(await _storage.read(key: _recoveryFailedAttemptsKey) ?? '') ?? 0) + 1;
    await _storage.write(key: _recoveryFailedAttemptsKey, value: '$attempts');
    if (attempts >= 5) {
      final exponent = (attempts - 5).clamp(0, 5).toInt();
      final until = DateTime.now().toUtc().add(Duration(seconds: 30 * (1 << exponent)));
      await _storage.write(key: _recoveryLockUntilKey, value: until.toIso8601String());
    }
  }
  ...
  void _validatePin(String pin) {
    if (!RegExp(r'^\d{4,8}$').hasMatch(pin)) throw ArgumentError('PIN harus 4–8 digit.');
  }

  String _generateRecoveryCode() {
    final random = Random.secure();
    return List<String>.generate(16, (_) => _recoveryAlphabet[random.nextInt(_recoveryAlphabet.length)]).join();
  }

  String _normalizeRecoveryCode(String value) => value.toUpperCase().replaceAll(RegExp(r'[^A-Z0-9]'), '');
  bool _isValidRecoveryCode(String value) => value.length == 16 && value.split('').every(_recoveryAlphabet.contains);
  String _formatRecoveryCode(String value) => [value.substring(0, 4), value.substring(4, 8), value.substring(8, 12), value.substring(12, 16)].join('-');
  ...
  Future<List<int>> _deriveSecretHash(String secret, List<int> salt) async {
    ...
    final key = await kdf.deriveKeyFromPassword(password: secret, nonce: salt);
