...
class BackupService {
  ...
  Future<List<File>> listLocalRecoveryGenerations() async {
    ...
    final files = await dir
        .list()
        .where(
          (entity) => entity is File && entity.path.endsWith('.arusrecovery'),
        )
        .cast<File>()
        .toList();
  ...
  Future<_DecodedBackup> _decodePortableBackup(
    ...
    if (size <= 0 || size > maxBackupBytes) {
      throw const FormatException(
        'Ukuran backup tidak valid atau terlalu besar.',
      );
    }
  ...
  Future<_DecodedBackup> _decodeLocalRecovery(File file) async {
    ...
    if (nonce.length != 12 || macBytes.length != 16 || cipher.isEmpty) {
      throw const FormatException(
        'Kriptografi titik pemulihan lokal tidak valid.',
      );
    }
    final clear = await AesGcm.with256bits().decrypt(
      SecretBox(cipher, nonce: nonce, mac: Mac(macBytes)),
  ...
  void _validatePassphrase(String value) {
    if (value.length < 8)
      throw ArgumentError('Passphrase backup minimal 8 karakter.');
  }
  ...
  void _validateLedger(dynamic db) {
    ...
    final orphanLegs =
        db
                .select(
                  'SELECT COUNT(*) AS c FROM transaction_legs l LEFT JOIN transactions t ON t.id=l.transaction_id WHERE t.id IS NULL',
                )
                .first['c']
            as int;
    final orphanSplits =
        db
                .select(
                  'SELECT COUNT(*) AS c FROM transaction_splits s LEFT JOIN transactions t ON t.id=s.transaction_id WHERE t.id IS NULL',
                )
                .first['c']
            as int;
    final postedWithoutLeg =
        db
                .select(
                  "SELECT COUNT(*) AS c FROM transactions t WHERE t.status='POSTED' AND t.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM transaction_legs l WHERE l.transaction_id=t.id)",
                )
                .first['c']
            as int;
    final badCurrency =
        db
                .select(
                  'SELECT COUNT(*) AS c FROM transaction_legs l JOIN transactions t ON t.id=l.transaction_id WHERE l.currency<>t.primary_currency',
                )
                .first['c']
            as int;
    final nonIdr =
    ...
    final invalidTxEnum =
        db
                .select(
                  "SELECT COUNT(*) AS c FROM transactions WHERE type NOT IN ('EXPENSE','INCOME','TRANSFER','REFUND','ADJUSTMENT','OPENING_BALANCE','CREDIT_CARD_PAYMENT','LOAN_DISBURSEMENT','LOAN_PAYMENT') OR status NOT IN ('POSTED','DRAFT','SCHEDULED','VOIDED')",
                )
                .first['c']
            as int;
    final invalidRecurring =
        db
                .select(
                  "SELECT COUNT(*) AS c FROM recurring_rules WHERE mode NOT IN ('REMINDER_ONLY','AUTO_CREATE','AUTO_CREATE_DRAFT') OR day_of_month NOT BETWEEN 1 AND 31",
                )
                .first['c']
            as int;
    final invalidBill =
        db
                .select(
                  "SELECT COUNT(*) AS c FROM bills WHERE status NOT IN ('UPCOMING','DUE','PAID','SKIPPED','OVERDUE')",
                )
                .first['c']
            as int;
    final invalidBudgetSemantics =
        db.select("""SELECT COUNT(*) AS c FROM budgets b
      ...
      """).first['c']
            as int;
    final invalidBillSemantics =
        db.select("""SELECT COUNT(*) AS c FROM bills
      WHERE trim(name)='' OR expected_amount_minor<=0 OR currency<>'IDR' OR length(due_date)<>10
      """).first['c']
            as int;
    final invalidRecurringSemantics =
        db.select("""SELECT COUNT(*) AS c FROM recurring_rules r
      ...
      """).first['c']
            as int;
    final invalidAccountSemantics =
        db.select("""SELECT COUNT(*) AS c FROM accounts
      ...
      """).first['c']
            as int;
    final invalidCategorySemantics =
        db.select("""SELECT COUNT(*) AS c FROM categories c
      LEFT JOIN categories p ON p.id=c.parent_id
      WHERE trim(c.name)='' OR c.parent_id=c.id OR
        (c.parent_id IS NOT NULL AND (p.id IS NULL OR p.type<>c.type))
      """).first['c']
            as int;
    final invalidImportFingerprint =
        db.select("""SELECT COUNT(*) AS c FROM import_fingerprints
      WHERE length(fingerprint)<>64 OR fingerprint GLOB '*[^0-9a-f]*'
      """).first['c']
            as int;
    final invalidMoneyRange =
        db.select("""SELECT (
    ...
    final invalidDateSemantics =
      ...
      (SELECT COUNT(*) FROM import_fingerprints WHERE julianday(created_at) IS NULL)
            ...
            as int;
    final invalidRefundSemantics =
        db.select("""SELECT COUNT(*) AS c FROM transactions r
      ...
      WHERE r.type='REFUND' AND r.deleted_at IS NULL AND (
        ...
        (SELECT s.category_id FROM transaction_splits s WHERE s.transaction_id=o.id LIMIT 1)
            ...
            as int;
    final invalidRecurringOccurrenceSemantics =
        db.select("""SELECT COUNT(*) AS c FROM recurring_occurrences ro
      JOIN recurring_rules rr ON rr.id=ro.rule_id
    ...
    final duplicateAccounts =
        db
                .select(
                  "SELECT COUNT(*) AS c FROM (SELECT lower(name) n FROM accounts WHERE archived_at IS NULL GROUP BY lower(name) HAVING COUNT(*)>1)",
                )
                .first['c']
            as int;
    final duplicateCategories =
        db
                .select(
                  "SELECT COUNT(*) AS c FROM (SELECT type,lower(name) n FROM categories WHERE archived_at IS NULL GROUP BY type,lower(name) HAVING COUNT(*)>1)",
                )
                .first['c']
            as int;
    final splitMismatch =
      ...
      WHERE t.type IN ('EXPENSE','INCOME','REFUND') AND t.deleted_at IS NULL
        AND COALESCE((SELECT SUM(s.amount_minor) FROM transaction_splits s WHERE s.transaction_id=t.id),0) <> t.primary_amount_minor""",
            ).first['c']
            as int;
    final badSplitCurrency =
        db
                .select(
                  'SELECT COUNT(*) AS c FROM transaction_splits s JOIN transactions t ON t.id=s.transaction_id WHERE s.currency<>t.primary_currency',
                )
                .first['c']
            as int;
    final invalidSimpleShape =
        db.select("""SELECT COUNT(*) AS c FROM transactions t
      ...
      WHERE t.type IN ('EXPENSE','INCOME','REFUND') AND t.deleted_at IS NULL AND NOT EXISTS (
        ...
        WHERE l.transaction_id=t.id AND (
          ...
          (t.type='REFUND' AND c.type='EXPENSE' AND (
            ...
            as int;
    final invalidNonCategorizedShape =
        db.select("""SELECT COUNT(*) AS c FROM transactions t
      ...
      WHERE t.type IN ('TRANSFER','ADJUSTMENT','OPENING_BALANCE','CREDIT_CARD_PAYMENT','LOAN_DISBURSEMENT','LOAN_PAYMENT')
        AND t.deleted_at IS NULL AND (
          ...
          (t.type IN ('ADJUSTMENT','OPENING_BALANCE') AND NOT EXISTS (
            ...
            as int;
    final invalidOriginalLink =
        db
                .select(
                  "SELECT COUNT(*) AS c FROM transactions WHERE deleted_at IS NULL AND ((type='REFUND' AND status='POSTED' AND original_transaction_id IS NULL) OR (type<>'REFUND' AND original_transaction_id IS NOT NULL))",
                )
                .first['c']
            as int;
    final invalidGroupShape =
        db.select("""SELECT COUNT(*) AS c FROM transaction_groups g
    ...
    final invalidGroupMembership =
      ...
      """).first['c']
            as int;
    final orphanPostedRefund =
        db.select(
              """SELECT COUNT(*) AS c FROM transactions r
      ...
      WHERE r.type='REFUND' AND r.status='POSTED' AND r.deleted_at IS NULL
        AND (o.id IS NULL OR o.type<>'EXPENSE' OR o.status<>'POSTED' OR o.deleted_at IS NOT NULL)""",
            ).first['c']
            as int;
    final overRefund =
        db.select(
              """SELECT COUNT(*) AS c FROM transactions o
      WHERE o.type='EXPENSE' AND o.status='POSTED' AND o.deleted_at IS NULL
        AND COALESCE((SELECT SUM(r.primary_amount_minor) FROM transactions r
          WHERE r.original_transaction_id=o.id AND r.type='REFUND' AND r.status='POSTED' AND r.deleted_at IS NULL),0) > o.primary_amount_minor""",
            ).first['c']
            as int;
    final invalidPaidBill =
        db.select("""SELECT COUNT(*) AS c FROM bills b
      ...
      WHERE b.status='PAID' AND (
        ...
        COALESCE((SELECT SUM(r.primary_amount_minor) FROM transactions r
          WHERE r.original_transaction_id=t.id AND r.type='REFUND' AND r.status='POSTED' AND r.deleted_at IS NULL),0) >= t.primary_amount_minor
            ...
            as int;
    final invalidUnpaidBillActivePayment =
        db.select("""SELECT COUNT(*) AS c FROM bills b
    ...
    if (fkErrors.isNotEmpty ||
        ...
        invalidRecurring != 0 ||
        invalidBill != 0 ||
        invalidBudgetSemantics != 0 ||
        invalidBillSemantics != 0 ||
        invalidRecurringSemantics != 0 ||
        invalidAccountSemantics != 0 ||
        invalidCategorySemantics != 0 ||
        invalidImportFingerprint != 0 ||
        invalidMoneyRange != 0 ||
        invalidDateSemantics != 0 ||
        invalidRefundSemantics != 0 ||
        invalidRecurringOccurrenceSemantics != 0 ||
        duplicateAccounts != 0 ||
        duplicateCategories != 0 ||
        splitMismatch != 0 ||
        invalidSimpleShape != 0 ||
        invalidSimpleSemantics != 0 ||
        invalidNonCategorizedShape != 0 ||
        invalidSpecialSemantics != 0 ||
        invalidOriginalLink != 0 ||
        invalidGroupShape != 0 ||
        invalidGroupSemantics != 0 ||
        invalidGroupMembership != 0 ||
        orphanPostedRefund != 0 ||
        overRefund != 0 ||
        invalidPaidBill != 0 ||
        invalidUnpaidBillActivePayment != 0) {
