...
class LocalFinanceRepository implements FinanceRepository {
  ...
  Future<void> _seedDefaultsIfNeeded() async {
    final accountCount =
        _db.select('SELECT COUNT(*) AS c FROM accounts').first['c'] as int;
    final categoryCount =
        _db.select('SELECT COUNT(*) AS c FROM categories').first['c'] as int;
    if (accountCount == 0) {
      await createAccount(
        name: 'Cash',
  ...
  void _requireMoneyMagnitude(int amountMinor, String label) {
    if (amountMinor < -kMaxMoneyMinor || amountMinor > kMaxMoneyMinor) {
      throw ArgumentError(
        '$label melebihi batas keamanan nominal local database.',
      );
    }
  }
  ...
  void _requirePositive(int amountMinor, String label) {
    _requireMoneyMagnitude(amountMinor, label);
    if (amountMinor <= 0)
      throw ArgumentError('$label harus lebih besar dari 0.');
  }

  void _requireSameCurrency(Map<String, Object?> a, Map<String, Object?> b) {
    if (a['currency'] != b['currency']) {
      throw StateError(
        'Operasi ini saat ini hanya mendukung account dengan currency yang sama.',
      );
    }
  }
  ...
  void _requireActiveAccount(Map<String, Object?> row) {
    if (row['archived_at'] != null)
      throw StateError('Account sudah diarsipkan.');
  }
  ...
  void _requireRefundDestination(Map<String, Object?> row) {
    ...
    final isCreditCard =
        row['account_class'] == 'LIABILITY' &&
        row['account_type'] == enumDbName(AccountType.creditCard);
    if (!isAsset && !isCreditCard) {
      throw StateError(
        'Refund hanya boleh masuk ke asset atau credit-card liability.',
      );
    }
  }
  ...
  void _requireRecurringExpenseAccount(Map<String, Object?> row) {
    ...
    if (!isAsset && !isCreditCard) {
      throw StateError(
        'Recurring expense hanya boleh memakai asset atau credit-card liability.',
      );
    }
  }
  ...
  void _requireSupportedCurrency(String currency) {
    if (currency != 'IDR') {
      throw StateError(
        'Versi local-only saat ini hanya mendukung IDR sampai engine multi-currency tersedia.',
      );
    }
  }
  ...
  void _requirePostableDate(DateTime value) {
    if (_isFutureLocalDate(value)) {
      throw StateError(
        'Transaksi masa depan tidak boleh langsung POSTED. Gunakan bill/recurring sampai fitur scheduled transaction tersedia.',
      );
    }
  }
  ...
  void _requireAccountTypeMatchesClass(
    ...
    final valid = accountClass == AccountClass.asset
        ? assetTypes.contains(accountType)
        : liabilityTypes.contains(accountType);
    if (!valid)
      throw ArgumentError(
        'Tipe account tidak sesuai dengan kelas asset/liability.',
      );
  }
    ...
    return rows
        .map(
          (r) => Account(
            ...
            accountClass: enumFromDb(
              r['account_class'] as String,
              AccountClass.values,
            ),
            accountType: enumFromDb(
              r['account_type'] as String,
              AccountType.values,
            ),
  ...
  Future<String> createAccount({
    ...
    final duplicate = _db.select(
      'SELECT id FROM accounts WHERE archived_at IS NULL AND lower(name)=lower(?) LIMIT 1',
      [trimmed],
    );
    if (duplicate.isNotEmpty)
      throw StateError('Nama account aktif sudah digunakan.');
    final id = _id();
    _database.transaction((db) {
      final now = _now();
      db.execute(
  ...
  Future<String> createCategory({
    ...
    final duplicate = _db.select(
      'SELECT id FROM categories WHERE archived_at IS NULL AND type=? AND lower(name)=lower(?) LIMIT 1',
      [enumDbName(type), trimmed],
    );
    if (duplicate.isNotEmpty)
  ...
  Future<String> createExpense({
    ...
    if (account['account_class'] == 'LIABILITY' &&
      ...
      throw StateError(
        'Expense langsung ke liability non-credit-card belum didukung.',
      );
    }
    final currency = account['currency'] as String;
    late String txId;
    _database.transaction((db) {
      txId = _insertTransaction(
        db,
      ...
      final delta = account['account_class'] == 'ASSET'
          ? -amountMinor
          : amountMinor;
      _insertLeg(
        db,
        ...
        delta,
        currency,
      ); // intended leg; only POSTED affects balance/report
  ...
  Future<String> createIncome({
    ...
    if (account['account_class'] != 'ASSET')
      throw StateError('Income normal harus masuk ke asset account.');
    final currency = account['currency'] as String;
    late String txId;
    _database.transaction((db) {
      txId = _insertTransaction(
  ...
  Future<String> createTransfer({
    ...
    if (sourceAccountId == destinationAccountId)
      throw ArgumentError('Source dan destination account tidak boleh sama.');
    ...
    if (source['account_class'] != 'ASSET' ||
        destination['account_class'] != 'ASSET') {
    ...
    _database.transaction((db) {
      ...
      if (feeMinor > 0) {
        ...
        final now = _now();
        db.execute(
          'INSERT INTO transaction_groups(id,group_type,created_at,updated_at,version) VALUES (?,?,?,?,1)',
          [groupId, 'TRANSFER_WITH_FEE', now, now],
        );
      }
      primaryId = _insertTransaction(
        db,
      ...
      if (feeMinor > 0) {
        final feeCategoryRows = db.select(
          "SELECT id FROM categories WHERE type='EXPENSE' AND archived_at IS NULL AND name='Biaya Transfer' LIMIT 1",
        );
        if (feeCategoryRows.isEmpty)
          throw StateError('Kategori Biaya Transfer tidak tersedia.');
        final feeTx = _insertTransaction(
          db,
          type: TransactionType.expense,
          status: TransactionStatus.posted,
        ...
        _insertSplit(
          db,
          feeTx,
          feeCategoryRows.first['id'] as String,
          feeMinor,
          currency,
        );
      }
  ...
  Future<String> createRefund({
    ...
    final originalRows = _db.select(
      'SELECT * FROM transactions WHERE id=? AND deleted_at IS NULL',
      [originalTransactionId],
    );
    if (originalRows.isEmpty)
      throw StateError('Transaksi original tidak ditemukan.');
    final original = originalRows.first;
    if (original['type'] != 'EXPENSE' || original['status'] != 'POSTED')
      throw StateError('Refund hanya dapat dibuat dari posted expense.');
    final refunded =
        _db.select(
    ...
    if (refunded + amountMinor > originalAmount)
    ...
    if (refundDate.isBefore(originalDate))
      throw StateError(
        'Tanggal refund tidak boleh mendahului transaksi original.',
      );
    final destination = _accountRow(destinationAccountId);
    _requireRefundDestination(destination);
    if (destination['currency'] != original['primary_currency'])
      throw StateError('Currency refund harus sama dengan transaksi original.');
    final splitRows = _db.select(
      'SELECT category_id FROM transaction_splits WHERE transaction_id=? LIMIT 1',
      [originalTransactionId],
    );
    if (splitRows.isEmpty)
      throw StateError('Kategori original tidak ditemukan.');
    ...
    _database.transaction((db) {
      ...
      final latestRefunded =
          db.select(
                "SELECT COALESCE(SUM(primary_amount_minor),0) AS total FROM transactions WHERE original_transaction_id=? AND type='REFUND' AND status='POSTED' AND deleted_at IS NULL",
                [originalTransactionId],
              ).first['total']
              as int;
      if (latestRefunded + amountMinor >
          (latestOriginal['primary_amount_minor'] as int)) {
        throw StateError(
          'Total refund tidak boleh melebihi transaksi original.',
        );
      }
      if (destination['currency'] != latestOriginal['primary_currency']) {
        throw StateError(
          'Currency refund harus sama dengan transaksi original.',
        );
      }
      ...
      final delta = destination['account_class'] == 'ASSET'
          ? amountMinor
          : -amountMinor;
  ...
  Future<String> createCreditCardPayment({
    ...
    if (source['account_class'] != 'ASSET')
      throw StateError('Sumber pembayaran harus asset.');
    if (card['account_type'] != enumDbName(AccountType.creditCard) ||
        card['account_class'] != 'LIABILITY') {
  ...
  Future<String> createLoanDisbursement({
    ...
    if (asset['account_class'] != 'ASSET' ||
        loan['account_class'] != 'LIABILITY' ||
        loan['account_type'] != enumDbName(AccountType.loan)) {
  ...
  Future<String> createLoanPayment({
    ...
    if (outstanding < 0)
      throw StateError(
        'Pinjaman memiliki credit balance yang harus direkonsiliasi sebelum pembayaran baru.',
      );
    if (principalMinor > outstanding)
      throw StateError('Pokok pembayaran melebihi outstanding pinjaman.');
    if (interestMinor > 0)
      _requireCategoryType(
        _categoryRow(interestCategoryId),
        CategoryType.expense,
      );
    if (feeMinor > 0)
      _requireCategoryType(_categoryRow(feeCategoryId), CategoryType.expense);
    final currency = asset['currency'] as String;
    late String primaryId;
    _database.transaction((db) {
      ...
      void childExpense(int amount, String categoryId, String childNote) {
        if (amount <= 0) return;
        final childId = _insertTransaction(
          db,
          type: TransactionType.expense,
          status: TransactionStatus.posted,
          amountMinor: amount,
      ...
      childExpense(
        interestMinor,
        interestCategoryId,
        note == null ? 'Bunga pinjaman' : 'Bunga pinjaman — $note',
      );
      childExpense(
        feeMinor,
        feeCategoryId,
        note == null ? 'Biaya pinjaman' : 'Biaya pinjaman — $note',
      );
    });
    return primaryId;
  }
  ...
  Future<String> reconcileAccount({
    ...
    if (reason.trim().isEmpty)
      throw ArgumentError('Alasan adjustment wajib diisi.');
  ...
  Future<void> updateSimpleTransaction({
    ...
    if (tx['transaction_group_id'] != null)
      throw StateError(
        'Composite transaction harus diedit melalui group editor.',
      );
    if (tx['type'] != 'EXPENSE' && tx['type'] != 'INCOME')
      throw StateError(
        'Hanya simple expense/income yang dapat diedit dengan fungsi ini.',
      );
    final refundTotal =
        _db.select(
              "SELECT COALESCE(SUM(primary_amount_minor),0) AS total FROM transactions WHERE original_transaction_id=? AND type='REFUND' AND status='POSTED' AND deleted_at IS NULL",
            ...
            as int;
    if (amountMinor < refundTotal)
      throw StateError(
        'Nominal baru tidak boleh lebih kecil dari refund aktif.',
      );
    final account = _accountRow(accountId);
    ...
    if (!isExpense && account['account_class'] != 'ASSET')
      throw StateError('Income harus masuk ke asset.');
    final currency = account['currency'] as String;
    _database.transaction((db) {
      ...
      if (latestRows.isEmpty ||
          latestRows.first['transaction_group_id'] != null ||
          (latestRows.first['type'] != 'EXPENSE' &&
              latestRows.first['type'] != 'INCOME')) {
        throw StateError(
          'Transaksi tidak lagi valid untuk diedit sebagai simple transaction.',
        );
      }
      final latestRefundTotal =
          db.select(
                "SELECT COALESCE(SUM(primary_amount_minor),0) AS total FROM transactions WHERE original_transaction_id=? AND type='REFUND' AND status='POSTED' AND deleted_at IS NULL",
                [transactionId],
              ).first['total']
      ...
      db.execute('DELETE FROM transaction_legs WHERE transaction_id=?', [
        transactionId,
      ]);
      db.execute('DELETE FROM transaction_splits WHERE transaction_id=?', [
        transactionId,
      ]);
      db.execute(
      ...
      int delta;
      if (isExpense) {
        delta = account['account_class'] == 'ASSET'
            ? -amountMinor
            : amountMinor;
      } else {
        delta = amountMinor;
      ...
      _insertLeg(
        ...
        delta,
        currency,
      ); // intended leg for DRAFT; balance uses POSTED only
  ...
  Future<void> postDraftTransaction(String transactionId) async {
    ...
    final tx = rows.first;
    if (tx['status'] != 'DRAFT')
      throw StateError('Hanya transaksi DRAFT yang dapat dicatat sekarang.');
    _requirePostableDate(DateTime.parse(tx['local_date'] as String));
    if (tx['type'] != 'EXPENSE' && tx['type'] != 'INCOME')
      throw StateError('Tipe draft belum didukung untuk posting.');
    final legs = _db.select(
      'SELECT l.*,a.archived_at FROM transaction_legs l JOIN accounts a ON a.id=l.account_id WHERE l.transaction_id=?',
      [transactionId],
    );
    final splits = _db.select(
      'SELECT s.*,c.archived_at FROM transaction_splits s JOIN categories c ON c.id=s.category_id WHERE s.transaction_id=?',
      [transactionId],
    );
    if (legs.length != 1 || splits.isEmpty)
      throw StateError('Draft tidak lengkap dan harus diedit terlebih dahulu.');
    if (legs.first['archived_at'] != null ||
        splits.any((x) => x['archived_at'] != null))
      throw StateError(
        'Account/kategori draft sudah diarsipkan. Edit draft terlebih dahulu.',
      );
    _database.transaction((db) {
      db.execute(
        "UPDATE transactions SET status='POSTED',updated_at=?,version=version+1 WHERE id=?",
        [_now(), transactionId],
      );
    });
  }
  ...
  Future<void> deleteTransaction(String transactionId) async {
    ...
    _database.transaction((db) {
      final targets = groupId == null
          ...
          : db.select(
              'SELECT id FROM transactions WHERE transaction_group_id=? AND deleted_at IS NULL',
              [groupId],
            );
      ...
      for (final id in targetIds) {
        ...
        if (activeRefunds > 0) {
          throw StateError(
            'Transaksi mempunyai refund aktif. VOID/hapus refund terlebih dahulu.',
          );
        }
      }
      ...
      for (final id in targetIds) {
        ...
        db.execute(
          "DELETE FROM transactions WHERE original_transaction_id=? AND type='REFUND' AND status='VOIDED'",
          [id],
        );
        // Preserve recurring history while detaching a transaction that the user explicitly deletes.
        db.execute(
          'UPDATE recurring_occurrences SET transaction_id=NULL WHERE transaction_id=?',
          [id],
        );
        db.execute(
      ...
      if (groupId != null) {
        db.execute(
          'DELETE FROM transaction_groups WHERE id=? AND NOT EXISTS (SELECT 1 FROM transactions WHERE transaction_group_id=?)',
          [groupId, groupId],
        );
      }
  ...
  Future<void> voidTransaction(
    ...
    if (trimmedReason.length < 3)
      throw ArgumentError('Alasan pembatalan minimal 3 karakter.');
    final rows = _db.select(
      'SELECT * FROM transactions WHERE id=? AND deleted_at IS NULL',
      [transactionId],
    );
    ...
    if (tx['status'] != 'POSTED')
      throw StateError('Hanya transaksi POSTED yang dapat dibatalkan/VOID.');
    ...
    _database.transaction((db) {
      final targets = groupId == null
          ? db.select(
              'SELECT id,note FROM transactions WHERE id=? AND deleted_at IS NULL',
              [transactionId],
            )
          : db.select(
              'SELECT id,note FROM transactions WHERE transaction_group_id=? AND deleted_at IS NULL',
              [groupId],
            );
      ...
      for (final row in targets) {
        ...
        if (activeRefunds > 0) {
          throw StateError(
            'Group transaksi mempunyai refund aktif. Batalkan/resolve refund terlebih dahulu.',
          );
        }
      }
      ...
      for (final row in targets) {
        ...
        final note = [
          if (oldNote != null && oldNote.isNotEmpty) oldNote,
          'VOID: $trimmedReason',
        ].join(' • ');
        db.execute(
          "UPDATE bills SET status=CASE WHEN due_date < ? THEN 'OVERDUE' WHEN due_date = ? THEN 'DUE' ELSE 'UPCOMING' END,paid_transaction_id=NULL,updated_at=? WHERE paid_transaction_id=?",
          [_localDate(_clock()), _localDate(_clock()), now, row['id']],
        );
        db.execute(
          "UPDATE transactions SET status='VOIDED',note=?,updated_at=?,version=version+1 WHERE id=?",
          [note, now, row['id']],
        );
      }
  ...
  Future<List<TransactionView>> listTransactions({
    ...
    final q = query?.trim();
    final where = <String>['t.deleted_at IS NULL', 't.group_primary=1'];
    final args = <Object?>[];
    ...
    if (filter?.accountId != null) {
      where.add(
        'EXISTS (SELECT 1 FROM transaction_legs fa WHERE fa.transaction_id=t.id AND fa.account_id=?)',
      );
      args.add(filter!.accountId);
    }
    if (filter?.categoryId != null) {
      where.add(
        'EXISTS (SELECT 1 FROM transaction_splits fs WHERE fs.transaction_id=t.id AND fs.category_id=?)',
      );
    ...
    args.add(limit);
    args.add(offset);
    final rows = _db.select('''SELECT t.*,
  ...
  Future<TransactionDetail> getTransactionDetail(String transactionId) async {
    final txRows = _db.select(
      ...
      FROM transactions t WHERE t.id=? AND t.deleted_at IS NULL LIMIT 1''',
      [transactionId],
    );
    ...
    final legRows = _db.select(
      'SELECT l.account_id,l.delta_minor,a.account_class,a.account_type FROM transaction_legs l JOIN accounts a ON a.id=l.account_id WHERE l.transaction_id=? ORDER BY l.rowid',
      [transactionId],
    );
    String? accountId;
    ...
    if (legRows.isNotEmpty) {
      switch (view.type) {
        case TransactionType.transfer:
          final source = legRows
              .where((x) => (x['delta_minor'] as int) < 0)
              .toList();
          final destination = legRows
              .where((x) => (x['delta_minor'] as int) > 0)
              .toList();
          accountId = source.isEmpty
              ? legRows.first['account_id'] as String
              : source.first['account_id'] as String;
          destinationAccountId = destination.isEmpty
              ? null
              : destination.first['account_id'] as String;
          break;
        case TransactionType.creditCardPayment:
          final assets = legRows
              .where((x) => x['account_class'] == 'ASSET')
              .toList();
          final liabilities = legRows
              .where((x) => x['account_class'] == 'LIABILITY')
              .toList();
          accountId = assets.isEmpty
              ? null
              : assets.first['account_id'] as String;
          destinationAccountId = liabilities.isEmpty
              ? null
              : liabilities.first['account_id'] as String;
          break;
        default:
    ...
    return TransactionDetail(
      ...
      categoryId: categoryRows.isEmpty
          ? null
          : categoryRows.first['category_id'] as String,
  ...
  Future<DashboardData> dashboard({DateTime? now}) async {
    ...
    int scalar(String sql, List<Object?> args) =>
        _db.select(sql, args).first['v'] as int;
    final expenseSql =
    ...
    return DashboardData(
      ...
      largestCategoryAmountMinor: catRows.isEmpty
          ? 0
          : catRows.first['total'] as int,
    );
  }
  ...
  Future<String> createBudget({
    ...
    if (end.isBefore(start))
      throw ArgumentError('Tanggal akhir budget tidak valid.');
    if (categoryId != null)
      _requireCategoryType(_categoryRow(categoryId), CategoryType.expense);
    final id = _id();
    final now = _now();
    _database.transaction((db) {
      db.execute(
        'INSERT INTO budgets(id,name,limit_minor,currency,category_id,period_start,period_end,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)',
        [
          id,
          trimmedName,
          limitMinor,
  ...
  Future<void> archiveBudget(String budgetId) async {
    final rows = _db.select(
      'SELECT * FROM budgets WHERE id=? AND archived_at IS NULL',
      [budgetId],
    );
    if (rows.isEmpty) return;
    _database.transaction((db) {
      final now = _now();
      db.execute('UPDATE budgets SET archived_at=?,updated_at=? WHERE id=?', [
        now,
        now,
        budgetId,
      ]);
  ...
  Future<List<BudgetModel>> listBudgets({DateTime? now}) async {
    ...
    final rows = _db.select(
      'SELECT * FROM budgets WHERE archived_at IS NULL AND period_start<=? AND period_end>=? ORDER BY name',
      [date, date],
    );
    final result = <BudgetModel>[];
    for (final r in rows) {
      final categoryId = r['category_id'] as String?;
      final effectiveEnd = (r['period_end'] as String).compareTo(date) > 0
          ? date
  ...
  bool _paymentFullyRefunded(dynamic db, String paymentTransactionId) {
    ...
    if (payment['status'] != 'POSTED' || payment['deleted_at'] != null)
      return true;
    final refunded =
        db.select(
  ...
  void _reconcileBillForPayment(
    dynamic db,
    String paymentTransactionId, {
    DateTime? now,
  }) {
    final bills = db.select(
      'SELECT id,due_date,status FROM bills WHERE paid_transaction_id=?',
      [paymentTransactionId],
    );
    ...
    for (final bill in bills) {
      ...
      final nextStatus = fullyRefunded
          ...
          : 'PAID';
      db.execute('UPDATE bills SET status=?,updated_at=? WHERE id=?', [
        nextStatus,
        stamp,
        bill['id'],
      ]);
  ...
  Future<String> createBill({
    ...
    _database.transaction((db) {
      db.execute(
        'INSERT INTO bills(id,name,expected_amount_minor,currency,due_date,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)',
        [
          id,
          trimmedName,
          expectedAmountMinor,
  ...
  Future<void> markBillStatus(String billId, BillStatus status) async {
    if (status != BillStatus.skipped) {
      throw StateError(
        'Status tagihan selain SKIPPED harus berasal dari tanggal jatuh tempo atau payBill().',
      );
    }
    _database.transaction((db) {
      final rows = db.select(
        'SELECT status,paid_transaction_id FROM bills WHERE id=?',
        [billId],
      );
      ...
      if (current['status'] == 'PAID' ||
        ...
        throw StateError(
          'Tagihan dengan pembayaran aktif tidak boleh dilewati.',
        );
      }
      db.execute('UPDATE bills SET status=?,updated_at=? WHERE id=?', [
        enumDbName(status),
  ...
  Future<String> payBill({
    ...
    if (account['account_class'] == 'LIABILITY' &&
        account['account_type'] != enumDbName(AccountType.creditCard)) {
      throw StateError(
        'Pembayaran tagihan hanya boleh dari asset atau credit card.',
      );
    }
    if (account['currency'] != bill['currency'])
      throw StateError('Currency pembayaran harus sama dengan tagihan.');
    final currency = bill['currency'] as String;
    late String txId;
    _database.transaction((db) {
      final latestBill = db.select(
        'SELECT status,paid_transaction_id FROM bills WHERE id=?',
        [billId],
      );
      if (latestBill.isEmpty) throw StateError('Tagihan tidak ditemukan.');
      final previousPaymentId =
          latestBill.first['paid_transaction_id'] as String?;
      if (latestBill.first['status'] == 'PAID') {
        throw StateError('Tagihan sudah dibayar.');
      }
      if (previousPaymentId != null &&
          !_paymentFullyRefunded(db, previousPaymentId)) {
      ...
      _insertSplit(db, txId, categoryId, amountMinor, currency);
      db.execute(
        'UPDATE bills SET status=?,paid_transaction_id=?,updated_at=? WHERE id=?',
        ['PAID', txId, _now(), billId],
      );
    });
    return txId;
  }
  ...
  Future<List<BillModel>> listBills({DateTime? now}) async {
    final current = now ?? _clock();
    final today = _localDate(current);
    final linkedPayments = _db.select(
      "SELECT paid_transaction_id FROM bills WHERE paid_transaction_id IS NOT NULL AND status<>'SKIPPED'",
    );
    for (final row in linkedPayments) {
      _reconcileBillForPayment(
        _db,
        row['paid_transaction_id'] as String,
        now: current,
      );
    }
    // Reconcile derived unpaid status in both directions so a corrected device clock heals planning state.
    _db.execute(
      "UPDATE bills SET status='UPCOMING',updated_at=? WHERE status IN ('DUE','OVERDUE') AND due_date > ?",
      [_now(), today],
    );
    _db.execute(
      "UPDATE bills SET status='DUE',updated_at=? WHERE status IN ('UPCOMING','OVERDUE') AND due_date = ?",
      [_now(), today],
    );
    _db.execute(
      "UPDATE bills SET status='OVERDUE',updated_at=? WHERE status IN ('UPCOMING','DUE') AND due_date < ?",
      [_now(), today],
    );
    final rows = _db.select(
      "SELECT * FROM bills WHERE status NOT IN ('PAID','SKIPPED') ORDER BY due_date LIMIT 50",
    );
    return rows
        .map(
          (r) => BillModel(
  ...
  @override
  Future<String> createRecurringExpenseDraft({
    required String name,
    required int amountMinor,
    required String accountId,
    required String categoryId,
    ...
    if (dayOfMonth < 1 || dayOfMonth > 31)
      throw ArgumentError('Tanggal recurring harus 1–31.');
    final account = _accountRow(accountId);
    final category = _categoryRow(categoryId);
    _requireRecurringExpenseAccount(account);
    _requireCategoryType(category, CategoryType.expense);
    if (account['currency'] != currency)
      throw StateError('Currency recurring tidak sama dengan account.');
    final now = _clock();
    final nextRun = _nextMonthlyOccurrence(
      now,
      dayOfMonth,
    ...
    _database.transaction((db) {
      db.execute(
        '''INSERT INTO recurring_rules(id,name,mode,amount_minor,currency,account_id,category_id,day_of_month,next_run,active,version,created_at,updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,1,1,?,?)''',
        [
          id,
          trimmedName,
          enumDbName(RecurringMode.autoCreateDraft),
          amountMinor,
          currency,
          accountId,
          categoryId,
  ...
  DateTime _nextMonthlyOccurrence(
    ...
    if (candidate.isBefore(dayStart) ||
        (!includeCurrentDay &&
            candidate.year == from.year &&
            candidate.month == from.month &&
            candidate.day == from.day)) {
  ...
  Future<void> setRecurringActive(String recurringRuleId, bool active) async {
    final rows = _db.select('SELECT * FROM recurring_rules WHERE id=?', [
      recurringRuleId,
    ]);
    ...
    _database.transaction((db) {
      db.execute(
        'UPDATE recurring_rules SET active=?,updated_at=?,version=version+1 WHERE id=?',
        [active ? 1 : 0, _now(), recurringRuleId],
      );
    });
  }
  ...
  Future<List<RecurringRuleModel>> listRecurringRules() async {
    final rows = _db.select(
      'SELECT * FROM recurring_rules ORDER BY active DESC,next_run',
    );
    return rows
        .map(
          (r) => RecurringRuleModel(
            id: r['id'] as String,
            name: r['name'] as String,
  ...
  Future<int> generateDueRecurring({DateTime? now}) async {
    ...
    final futureRules = _db.select(
      'SELECT id,day_of_month,next_run FROM recurring_rules WHERE active=1 AND next_run>?',
      [_localDate(current)],
    );
    for (final rule in futureRules) {
      final next = DateTime.parse(rule['next_run'] as String);
      final monthsAhead =
          (next.year - current.year) * 12 + (next.month - current.month);
      if (monthsAhead > 2) {
        final healed = _nextMonthlyOccurrence(
          DateTime(current.year, current.month, 1),
          rule['day_of_month'] as int,
          includeCurrentDay: true,
        );
        _db.execute(
          'UPDATE recurring_rules SET next_run=?,updated_at=? WHERE id=?',
          [_localDate(healed), _now(), rule['id']],
        );
      }
    }

    final rows = _db.select(
      'SELECT * FROM recurring_rules WHERE active=1 AND next_run<=? ORDER BY next_run',
      [_localDate(current)],
    );
    var generated = 0;
    for (final r in rows) {
      ...
      if (recurringAccount['currency'] != r['currency']) {
        throw StateError(
          'Recurring rule mempunyai currency yang tidak konsisten dengan account.',
        );
      }
      var scheduled = DateTime.parse(r['next_run'] as String);
      final monthGap =
          (current.year - scheduled.year) * 12 +
          (current.month - scheduled.month);
      ...
      while (!scheduled.isAfter(current) && safety < 24) {
        ...
        final existing = _db.select(
          'SELECT id FROM recurring_occurrences WHERE rule_id=? AND occurrence_key=?',
          [r['id'], occurrenceKey],
        );
        if (existing.isEmpty) {
          ...
          _database.transaction((db) {
            ...
            if (mode != RecurringMode.reminderOnly) {
              ...
              final delta = recurringAccount['account_class'] == 'ASSET'
            ...
            db.execute(
              'INSERT INTO recurring_occurrences(id,rule_id,occurrence_key,transaction_id,scheduled_for,created_at) VALUES (?,?,?,?,?,?)',
              [_id(), r['id'], occurrenceKey, txId, _iso(scheduled), _now()],
            );
          });
          generated++;
        }
        scheduled = _nextMonthlyOccurrence(
          DateTime(scheduled.year, scheduled.month, scheduled.day + 1),
          r['day_of_month'] as int,
          includeCurrentDay: true,
        );
      }
      _db.execute(
        'UPDATE recurring_rules SET next_run=?,updated_at=? WHERE id=?',
        [_localDate(scheduled), _now(), r['id']],
      );
    }
    return generated;
  }
    ...
    final deps = _db
      ...
      (SELECT COUNT(*) FROM transaction_legs l JOIN transactions t ON t.id=l.transaction_id
       WHERE l.account_id=? AND t.status='POSTED' AND t.deleted_at IS NULL AND t.local_date>?) AS future_posted_count''',
          [accountId, accountId, accountId, _localDate(_clock())],
        )
        .first;
    if ((deps['recurring_count'] as int) > 0 ||
        (deps['draft_count'] as int) > 0 ||
        (deps['future_posted_count'] as int) > 0) {
      throw StateError(
        'Account masih dipakai recurring/draft/transaksi masa depan. Selesaikan atau reassign terlebih dahulu.',
      );
    }
    final balance = _accountBalance(accountId);
    if (balance != 0) {
      throw StateError(
        'Account masih memiliki saldo. Pindahkan/rekonsiliasi saldo hingga 0 sebelum diarsipkan.',
      );
    }
    _database.transaction((db) {
      final now = _now();
      db.execute(
        'UPDATE accounts SET archived_at=?,updated_at=?,version=version+1 WHERE id=?',
        [now, now, accountId],
      );
    });
  }
    ...
    final deps = _db
      ...
      (SELECT COUNT(*) FROM transaction_splits s JOIN transactions t ON t.id=s.transaction_id
       WHERE s.category_id=? AND t.status='POSTED' AND t.deleted_at IS NULL AND t.local_date>?) AS future_posted_count''',
        ...
        .first;
    if ((deps['recurring_count'] as int) > 0 ||
        (deps['budget_count'] as int) > 0 ||
        (deps['draft_count'] as int) > 0 ||
        (deps['future_posted_count'] as int) > 0) {
      throw StateError(
        'Kategori masih dipakai budget/recurring/draft/transaksi masa depan. Reassign terlebih dahulu.',
      );
    }
    _database.transaction((db) {
      final now = _now();
      db.execute(
        'UPDATE categories SET archived_at=?,updated_at=?,version=version+1 WHERE id=?',
        [now, now, categoryId],
      );
    });
  }
  ...
  @override
  Future<Set<String>> existingImportFingerprints(
    Iterable<String> fingerprints,
  ) async {
    final unique = fingerprints
  ...
  @override
  Future<ImportCommitResult> importSimpleTransactionsAtomically(
    List<ImportTransactionDraft> rows,
  ) async {
    if (rows.isEmpty) {
    ...
    for (final row in rows) {
      if (!seenFingerprints.add(row.fingerprint)) {
        throw StateError(
          'File import mengandung fingerprint duplikat internal.',
        );
      }
    ...
    final categoriesById = _rowsByIds(
      'categories',
    ...
    for (final row in rows) {
      ...
      if (category == null)
        throw StateError('Kategori import tidak ditemukan.');
      _requireActiveAccount(account);
      ...
      if (row.type == TransactionType.expense) {
        ...
        if (account['account_class'] == 'LIABILITY' &&
            account['account_type'] != enumDbName(AccountType.creditCard)) {
          throw StateError(
            'Expense ke liability non-credit-card tidak didukung.',
          );
        }
      } else {
        _requireCategoryType(category, CategoryType.income);
    ...
    final knownFingerprints = await existingImportFingerprints(
      seenFingerprints,
    );
    ...
    for (
      ...
      for (final existingRow in existing) {
        existingSourceRows[existingRow['id'] as String] =
            Map<String, Object?>.from(existingRow);
      }
    }
        ...
        if (sourceId != null) {
          ...
          if (existing != null) {
            final same =
                ...
                DateTime.parse(
                  existing['occurred_at_utc'] as String,
                ).toUtc().isAtSameMomentAs(row.occurredAt.toUtc());
            if (!same) {
              throw StateError(
    ...
    return ImportCommitResult(imported: imported, skippedDuplicates: skipped);
  }
  ...
  Map<String, Map<String, Object?>> _rowsByIds(
    ...
    for (
      ...
      for (final selectedRow in selected) {
        result[selectedRow['id'] as String] = Map<String, Object?>.from(
          selectedRow,
        );
      }
    }
