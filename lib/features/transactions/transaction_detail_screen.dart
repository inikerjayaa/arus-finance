...
class _TransactionDetailScreenState extends State<TransactionDetailScreen> {
  ...
  Future<void> _load() async {
    try {
      final detail = await AppScope.of(
        context,
      ).repository.getTransactionDetail(widget.transactionId);
      if (!mounted) return;
      setState(() {
        _detail = detail;
        _loading = false;
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => [
              ...
              if (d.canRefund)
                const PopupMenuItem(
                  value: 'refund',
                  child: Text('Buat refund'),
                ),
              if (tx.status == TransactionStatus.posted)
                const PopupMenuItem(
                  value: 'void',
                  child: Text('Batalkan (VOID)'),
                ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    Money.format(tx.amountMinor, currency: tx.currency),
                    style: theme.textTheme.displaySmall?.copyWith(
                      fontWeight: FontWeight.w800,
                    ),
                  ),
                  if (tx.childExpenseMinor > 0) ...[
                    const SizedBox(height: 8),
                    Text(
                      'Biaya terkait: ${Money.format(tx.childExpenseMinor, currency: tx.currency)}',
                    ),
                  ],
                ],
              ),
          ...
          Card(
            child: Column(
              children: [
                _row(
                  'Tanggal',
                  '${tx.occurredAt.day}/${tx.occurredAt.month}/${tx.occurredAt.year}',
                ),
                const Divider(height: 1),
                _row('Account', tx.accountName),
                if (tx.destinationAccountName != null) ...[
                  const Divider(height: 1),
                  _row('Tujuan', tx.destinationAccountName!),
                ],
                if (tx.categoryName != null) ...[
                  const Divider(height: 1),
                  _row('Kategori', tx.categoryName!),
                ],
                const Divider(height: 1),
                ...
                if (d.refundedMinor > 0) ...[
                  ...
                  _row(
                    'Sudah direfund',
                    Money.format(d.refundedMinor, currency: tx.currency),
                  ),
                ],
                if (tx.note?.isNotEmpty == true) ...[
                  const Divider(height: 1),
                  _row('Catatan', tx.note!),
                ],
              ],
            ),
          ),
  ...
  Future<void> _edit() async {
    ...
    final categories = d.view.type == TransactionType.expense
        ? c.expenseCategories
        : c.incomeCategories;
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: () async {
                    final lastDate = d.view.status == TransactionStatus.posted
                        ? DateTime.now()
                        : DateTime(2100);
                    final initialDate = date.isAfter(lastDate)
                        ? lastDate
                        : date;
                ...
                FilledButton(
                  onPressed: () async {
                    ...
                    final result = await c.run(
                      () => c.repository.updateSimpleTransaction(
  ...
  Future<void> _refund() async {
    ...
    final assetsAndCards = c.accounts
        .where(
          (a) =>
              a.accountClass == AccountClass.asset ||
              a.accountType == AccountType.creditCard,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'Maksimal ${Money.format(remaining, currency: d.view.currency)}',
              ),
              const SizedBox(height: 12),
              TextField(
                controller: amount,
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(labelText: 'Refund masuk ke'),
                items: assetsAndCards
                    .map(
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                final parsed = Money.parseIdr(amount.text);
                if (parsed == null || parsed <= 0 || destinationId == null)
                  return;
                final result = await c.run(
                  () => c.repository.createRefund(
    ...
    final reason = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              'VOID mengeluarkan efek transaksi dari saldo/laporan tetapi tetap menyimpan jejak transaksi.',
            ),
            const SizedBox(height: 12),
            TextField(
              controller: input,
  ...
  Future<void> _delete() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Transaksi akan dihapus dari database lokal dan efeknya dikeluarkan dari saldo/laporan. Tindakan ini tidak dapat dibatalkan tanpa backup. Gunakan VOID bila jejak pembatalan perlu tetap terlihat.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
