...
class _RecoveryScreenState extends State<RecoveryScreen> {
  ...
  Future<void> _loadRecoveryState() async {
    try {
      _latestLocalRecovery = await _backup.latestLocalRecoveryAt();
    } catch (_) {
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Arus tidak akan menimpa database yang gagal dibuka. Pilih jalur pemulihan yang tersedia di bawah ini.',
                    textAlign: TextAlign.center,
                  ...
                  FilledButton.icon(
                    ...
                    label: const Text('Coba buka database lagi'),
                  ),
                  const SizedBox(height: 10),
                  OutlinedButton.icon(
                    onPressed:
                  ...
                  const SizedBox(height: 10),
                  OutlinedButton.icon(
                    onPressed: _busy ? null : _restorePortable,
                    icon: const Icon(Icons.folder_open_outlined),
                    label: const Text('Pulihkan dari file .arusbackup'),
                  ),
                  if (_busy) ...[
                    const SizedBox(height: 18),
                    Semantics(
                  ...
                  const SizedBox(height: 16),
                  Text(
                    'Database lama dipindahkan ke area quarantine sebelum replacement dimulai. Jika recovery terputus sebelum replacement tervalidasi, startup berikutnya mengembalikan database lama.',
                    textAlign: TextAlign.center,
  ...
  Future<void> _retry() async {
    await _runRecovery(() async {
      ...
      if (widget.controller.errorMessage != null) {
        throw StateError(widget.controller.errorMessage!);
      }
  ...
  Future<void> _restoreLocal() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Arus akan menyimpan database yang gagal dibuka di quarantine, membangun database baru, lalu memasukkan snapshot terenkripsi terbaru. Replacement hanya dianggap sah setelah seluruh integrity check lulus.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _restorePortable() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'File akan didekripsi dan diperiksa terlebih dahulu. Database lama tidak ditimpa langsung; Arus menggunakan quarantine + validated replacement.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
