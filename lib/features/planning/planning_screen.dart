...
class PlanningScreen extends StatelessWidget {
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        PopupMenuButton<String>(
                          tooltip: 'Aksi budget ${b.name}',
                          onSelected: (v) {
                            if (v == 'archive')
                              _archiveBudget(context, b.id, b.name);
                          },
                          itemBuilder: (_) => const [
                            PopupMenuItem(
                              value: 'archive',
                              child: Text('Arsipkan budget'),
                            ),
                          ],
                        ),
                      ],
                    ...
                    LinearProgressIndicator(
                      value: b.progress.clamp(0.0, 1.0).toDouble(),
                    ),
                    const SizedBox(height: 10),
                    Text(
                      '${Money.format(b.actualMinor)} dari ${Money.format(b.limitMinor)} • sisa ${Money.format(b.remainingMinor)}',
                    ),
                    const SizedBox(height: 4),
                    Text(
                      'Aman/hari ${Money.format(b.safePerDayMinor(DateTime.now()))}',
          ...
          ...c.bills.map(
            (b) => Card(
              child: ListTile(
                title: Text(b.name),
                subtitle: Text(
                  '${b.dueDate.day}/${b.dueDate.month}/${b.dueDate.year} • ${b.status.name}\n${Money.format(b.expectedAmountMinor, currency: b.currency)}',
                ),
                trailing: PopupMenuButton<String>(
                  tooltip: 'Aksi tagihan ${b.name}',
                  onSelected: (v) {
                    if (v == 'paid')
                      _payBill(context, b.id, b.name, b.expectedAmountMinor);
          ...
          ...c.recurring.map(
            (r) => Card(
              child: SwitchListTile(
                title: Text(r.name),
                subtitle: Text(
                  '${r.mode.name} • berikutnya ${r.nextRun.day}/${r.nextRun.month}/${r.nextRun.year} • ${Money.format(r.amountMinor, currency: r.currency)}',
                ),
                value: r.active,
                onChanged: (value) =>
                    c.run(() => c.repository.setRecurringActive(r.id, value)),
              ),
            ),
          ),
      ],
  ...
  Future<void> _archiveBudget(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Budget tidak akan tampil sebagai budget aktif. Histori transaksi tidak dihapus.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _skipBill(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Tagihan akan ditandai SKIPPED dan tidak dicatat sebagai pembayaran.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
          ...
          FilledButton(
            onPressed: () => Navigator.pop(ctx, true),
            child: const Text('Lewati tagihan'),
          ),
        ],
      ),
    );
    if (ok == true) {
      await c.run(
        () => c.repository.markBillStatus(billId, BillStatus.skipped),
      );
    }
  }
              ...
              FilledButton(
                onPressed: () async {
                  ...
                  final result = await c.run(
                    () => c.repository.createBudget(
                      name: name.text,
                      limitMinor: Money.parseIdr(amount.text) ?? 0,
  ...
  Future<void> _payBill(
    ...
    final c = AppScope.of(context);
    final paymentAccounts = c.accounts
        .where(
          (a) =>
    ...
    var categoryId = c.expenseCategories
        .firstWhere(
          (x) => x.name == 'Tagihan',
          orElse: () => c.expenseCategories.first,
        )
        .id;
    await showModalBottomSheet<void>(
      context: context,
            ...
            child: Column(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  ...
                  label: Text(
                    'Jatuh tempo ${due.day}/${due.month}/${due.year}',
                  ),
                ),
                const SizedBox(height: 18),
                FilledButton(
                  onPressed: () async {
                    final result = await c.run(
                      () => c.repository.createBill(
                        name: name.text,
                        expectedAmountMinor: Money.parseIdr(amount.text) ?? 0,
                        dueDate: due,
                      ),
                    );
                    if (ctx.mounted && result != null) Navigator.pop(ctx);
  ...
  Future<void> _addRecurring(BuildContext context) async {
    final c = AppScope.of(context);
    final recurringAccounts = c.accounts
        .where(
          (a) =>
              a.accountClass == AccountClass.asset ||
              a.accountType == AccountType.creditCard,
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(
                    labelText: 'Tanggal tiap bulan (1–31)',
                  ),
                ),
                const SizedBox(height: 18),
                FilledButton(
                  onPressed: () async {
                    final result = await c.run(
                      () => c.repository.createRecurringExpenseDraft(
                        name: name.text,
                        amountMinor: Money.parseIdr(amount.text) ?? 0,
                        accountId: accountId,
                        categoryId: categoryId,
                        dayOfMonth: int.tryParse(day.text) ?? 1,
                      ),
                    );
                    if (ctx.mounted && result != null) Navigator.pop(ctx);
                  },
                  child: const Text('Simpan sebagai draft otomatis'),
                ),
              ],
            ),
          ),
