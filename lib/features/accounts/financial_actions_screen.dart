...
class FinancialActionsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final assets = c.accounts
        .where((a) => a.accountClass == AccountClass.asset)
        .toList();
    final cards = c.accounts
        .where((a) => a.accountType == AccountType.creditCard)
        .toList();
    final loans = c.accounts
        .where((a) => a.accountType == AccountType.loan)
        .toList();
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Text(
            'Settlement liability dipisahkan dari transfer biasa agar laporan tidak double-count.',
          ...
          _ActionCard(
            ...
            description:
                'Asset berkurang dan utang kartu kredit berkurang. Tidak menjadi pengeluaran kedua kali.',
            enabled: assets.isNotEmpty && cards.isNotEmpty,
            disabledReason: cards.isEmpty
                ? 'Buat account Credit Card terlebih dahulu.'
                : 'Buat asset account terlebih dahulu.',
          ...
          _ActionCard(
            ...
            description:
                'Asset bertambah dan liability bertambah dengan nilai yang sama. Tidak dianggap pemasukan.',
            enabled: assets.isNotEmpty && loans.isNotEmpty,
            disabledReason: loans.isEmpty
                ? 'Buat account Loan terlebih dahulu.'
                : 'Buat asset account terlebih dahulu.',
          ...
          _ActionCard(
            ...
            description:
                'Pokok mengurangi liability; bunga dan fee masuk pengeluaran secara terpisah.',
            enabled:
                assets.isNotEmpty &&
                loans.isNotEmpty &&
                c.expenseCategories.isNotEmpty,
            disabledReason: loans.isEmpty
                ? 'Buat account Loan terlebih dahulu.'
                : 'Data account/kategori belum siap.',
            onTap: () =>
                _loanPayment(context, assets, loans, c.expenseCategories),
          ),
        ],
      ),
    );
  ...
  Future<void> _creditCardPayment(
                ...
                FilledButton(
                  onPressed: () async {
                    ...
                    if (parsed > card.balanceMinor && card.balanceMinor >= 0) {
                      final proceed = await _confirmOverpay(
                        context,
                        card.balanceMinor,
                        parsed,
                      );
                    ...
                    final result = await c.run(
                      () => c.repository.createCreditCardPayment(
                        amountMinor: parsed,
              ...
              children: [
                ...
                FilledButton(
                  onPressed: () async {
                    ...
                    if (parsed == null || parsed <= 0) return;
                    final result = await c.run(
                      () => c.repository.createLoanDisbursement(
                        amountMinor: parsed,
  ...
  Future<void> _loanPayment(
    ...
    var assetId = assets.first.id;
    var loanId = loans.first.id;
    var interestCategoryId = categories
        .firstWhere(
          (x) => x.name == 'Bunga Pinjaman',
          orElse: () => categories.first,
        )
        .id;
    var feeCategoryId = categories
        .firstWhere(
          (x) => x.name == 'Biaya Pinjaman',
          orElse: () => categories.first,
        )
        .id;
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (v) => setState(
                    () => interestCategoryId = v ?? interestCategoryId,
                  ),
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  ...
                  items: categories
                      ...
                      .toList(),
                  onChanged: (v) =>
                      setState(() => feeCategoryId = v ?? feeCategoryId),
                ),
                const SizedBox(height: 12),
                TextField(
                  controller: note,
                  ...
                  onPressed: () async {
                    ...
                    if (p > loan.balanceMinor && loan.balanceMinor >= 0) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Pokok pembayaran melebihi outstanding pinjaman.',
                          ),
                        ),
                      );
                      return;
                    ...
                    final result = await c.run(
                      () => c.repository.createLoanPayment(
                        principalMinor: p,
                        interestMinor: i,
                        feeMinor: f,
    ...
    items: values
        .map(
          (a) => DropdownMenuItem(
            value: a.id,
            child: Text(
              '${a.name} • ${Money.format(a.balanceMinor, currency: a.currency)}',
            ),
          ),
        )
        .toList(),
  ...
  Future<bool> _confirmOverpay(
    ...
    return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text(
              'Outstanding ${Money.format(outstanding)}. Pembayaran ${Money.format(payment)} akan membuat saldo kredit pada kartu.',
            ),
            actions: [
              TextButton(
